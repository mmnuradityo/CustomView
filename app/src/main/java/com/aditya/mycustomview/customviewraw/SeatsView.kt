...
import com.aditya.mycustomview.toDp
import com.aditya.mycustomview.customviewraw.data.Seat
import com.aditya.mycustomview.customviewraw.data.SeatDataStore
...
class SeatsView: View {
    ...
    init {
        seats.addAll(SeatDataStore.seats)
    }

    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        super.onMeasure(widthMeasureSpec, heightMeasureSpec)
        val with = getDefaultSize(suggestedMinimumWidth, widthMeasureSpec)
        val height = getDefaultSize(suggestedMinimumHeight, widthMeasureSpec)

        val halfOfWidth = with / 2
        ...
        for (i in seats.indices) {
            if (i.mod(2) == 0) {
                xToSave = halfOfWidth - 300F
                yToSave = halfOfHeight + value
            } else {
                xToSave = halfOfWidth + 100F
                yToSave = halfOfHeight + value
                value += 300F
            }
